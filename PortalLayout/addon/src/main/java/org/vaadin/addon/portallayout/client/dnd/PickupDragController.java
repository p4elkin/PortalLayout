 ...
 * the License.
 */
package org.vaadin.addon.portallayout.client.dnd;

import java.util.ArrayList;
import java.util.HashMap;

import org.vaadin.addon.portallayout.client.dnd.drop.BoundaryDropController;
import org.vaadin.addon.portallayout.client.dnd.drop.DropController;
...
import org.vaadin.addon.portallayout.client.dnd.util.DragClientBundle;
import org.vaadin.addon.portallayout.client.dnd.util.Location;
import org.vaadin.addon.portallayout.client.dnd.util.WidgetArea;
import org.vaadin.addon.portallayout.client.dnd.util.WidgetLocation;
...
public class PickupDragController extends AbstractDragController {
    ...
    public void dragMove() {
        ...
        if (getBehaviorConstrainedToBoundaryPanel()) {
            desiredLeft = Math
                    .max(0, Math.min(desiredLeft, dropTargetClientWidth - context.draggable.getOffsetWidth()));
            desiredTop = Math
                    .max(0, Math.min(desiredTop, dropTargetClientHeight - context.draggable.getOffsetHeight()));
        }
    ...
    public void resetCache() {
        ...
        // VConsole.log("Cache invalidated in " + (System.currentTimeMillis() -
        // time));
    }
        ...
        for (Widget widget : context.selectedWidgets) {
            ...
            if (info.initialDraggableParent instanceof AbsolutePanel) {
                ((AbsolutePanel) info.initialDraggableParent).add(widget,
                        info.initialDraggableParentLocation.getLeft(), info.initialDraggableParentLocation.getTop());
